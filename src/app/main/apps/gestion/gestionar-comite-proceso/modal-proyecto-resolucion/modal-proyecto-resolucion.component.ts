...
export class ModalProyectoResolucionComponent implements OnInit {
  ...
  private validBuildData() {
    ...
    this.dataService.GestionProcesos().getRegimenGrupoAccion(this.data.proceso.idProceso, CodigoGrupoAccion.Comite, CodigoAccion.ConformacionComite)
      .pipe(
        catchError((e) => {
          return of(e);
        }),
  ...
  guardarDocumentoSustentoYGenerarProyectoResolucion() {
    ...
    objGenerarProyectoResolucion.append('idAlcanceProceso', this.data.permisoComite.idAlcanceProceso);
    ...
    this.dataService.Message().msgConfirm(MESSAGE_GESTION.M29, () => {
      ...
      this.dataService.GestionProcesos().crearProyectoResolucion(objGenerarProyectoResolucion)
        .pipe(
