...
@Component({
  selector: 'minedu-consolidado-centro-regimen',
  templateUrl: './consolidado-centro-regimen.component.html',
  styleUrls: ['./consolidado-centro-regimen.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: mineduAnimations
})
export class ConsolidadoCentroRegimenComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  displayedColumns: string[] = [
    ...
    'regimenLaboral',
    'condicionNombrados',
    'condicionContratados',
    'condicionContratoPlazoIndeterminado',
    'condicionOtros',
    'situacionEnActividad',
    'situacionDesignados',
    'situacionEncargados',
    'situacionConLicencia',
    'situacionOtros',
    'plazasOcupadas',
    'plazasVacantes',
    'totalPlazas'
  ...
  setParametrosConsulta() {
    ...
    this.consulta = {
      codigoTipoReporteCAP: CodigoTipoReporteCAP.CONSOLIDADO_SP_X_CENTRO_REGIMEN,
    ...
    this.dataService.ReportesCAP().GenerarReporteCAP(this.consulta)
      ...
      .subscribe(codigoDocumentoGenerado => {
        if (codigoDocumentoGenerado) {
          ...
          //this.handlePreview("Consolidado por Centro Trabajo y Régimen", this.pdfFromBase64(codigoDocumentoGenerado.cadenaBase64), "Consolidado por Centro Trabajo y Régimen");
        }
      });
  }
  ...
  handleExportar() {
    if (this.consulta) {
      this.cargarLoading();
      this.dataService.ReportesCAP().exportarConsolidadoCentroRegimen(this.consulta).pipe(
        catchError((error: HttpErrorResponse) => {
          this.dataService.Message().msgWarning(error.error.messages[0]); return of(null);
        }),
        finalize(() => { this.descargarLoading(); })
        ...
        if (response) {
          ...
          descargarExcel(response, `Reporte Consolidado servidores públicos por Cent. Trab. y Reg. Laboral ${anio} ${subinstancia} ${tipoCentroTrabajo}.xlsx`);
        }
      });
    } else {
      this.dataService.Message().msgWarning(MESSAGE_REPORTES_CAP.M01, () => { });
  ...
  handleVerReporteCAP(codigoDocumentoGenerado: string) {
    ...
    this.dataService.Documento().descargar(codigoDocumentoGenerado)
      .pipe(
        ...
        if (response) {
          this.handlePreview("Consolidado por Centro Trabajo y Régimen Laboral", response, "Consolidado por Centro Trabajo y Régimen Laboral");
        }
      });
  }
...
export class ReporteDataSource extends DataSource<any>{
  ...
  load(data: any, pageIndex: number, pageSize: number, autoSearch: boolean) {
    this._loadingChange.next(true);
    this.dataService.Spinner().show("sp6");
    this.dataService.ReportesCAP().consultarConsolidadoCentroRegimen(data, pageIndex, pageSize).pipe(
      catchError((error: HttpErrorResponse) => {
        if (!autoSearch){
          this._totalRows = 0;
