...
@Component({
  selector: 'minedu-cuadro-asignacion-personal',
  templateUrl: './cuadro-asignacion-personal.component.html',
  styleUrls: ['./cuadro-asignacion-personal.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: mineduAnimations
})
export class CuadroAsignacionPersonalComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  buildForm() {
    ...
    this.form.get("idRegimenLaboral").valueChanges.subscribe((value) => {
      this.areasDesempenioLaboral = [];
      this.form.patchValue({
          idAreaDesempenioLaboral: "-1",
      });
      if (value && value > 0) {
          this.loadAreaDesempenioLaboral(value, true);
      }
    });
    
    this.form.get("idAreaDesempenioLaboral").valueChanges.subscribe((value) => {
      const idRegimenLaboral = this.form.controls["idRegimenLaboral"].value;
      this.tiposCargo = [];
      this.form.patchValue({ idTipoCargo: "-1" });
      if (value && value > 0) {
          this.loadTipoCargo(idRegimenLaboral, value, true);
      }
    });

    this.form.get("idTipoCargo").valueChanges.subscribe((value) => {
        const idRegimenLaboral = this.form.controls["idRegimenLaboral"].value;
        const idAreaDesempenioLaboral = this.form.controls["idAreaDesempenioLaboral"].value;
        this.cargos = [];
        this.form.patchValue({ idCargo: "-1" });
        if (value && value > 0) {
            this.loadCargo(idRegimenLaboral, idAreaDesempenioLaboral, value, true);
        }
    });
  ...
  loadRegimenLaboral() {
    this.cargarLoading();
    this.dataService.ReportesCAP().getRegimenesLaborales(this.passport.codigoRol)
        .pipe(
          catchError((error: HttpErrorResponse) => {
            this.dataService.Message().msgWarning(error.error.messages[0]); return of(null);
          }),
  ...
  loadAreaDesempenioLaboral(idRegimenLaboral, activo) {
    this.cargarLoading();
    this.dataService.ReportesCAP().getAreasDesempenio(idRegimenLaboral, activo)
        .pipe(
          catchError((error: HttpErrorResponse) => {
            this.dataService.Message().msgWarning(error.error.messages[0]); return of(null);
          }),
          finalize(() => this.descargarLoading())
            ...
            if (response) { this.areasDesempenioLaboral = response; }
        });
  }

  loadTipoCargo(idRegimenLaboral, idAreaDesempenioLaboral, activo) {
    this.cargarLoading();
    this.dataService.ReportesCAP().getTiposCargo(idRegimenLaboral, idAreaDesempenioLaboral, activo)
        .pipe(
          catchError((error: HttpErrorResponse) => {
            this.dataService.Message().msgWarning(error.error.messages[0]); return of(null);
          }),
          finalize(() => this.descargarLoading())
            ...
            if (response) { this.tiposCargo = response; }
        });
  }

  loadCargo(idRegimenLaboral, idAreaDesempenioLaboral, idTipoCargo, activo) {
    this.cargarLoading();
    this.dataService.ReportesCAP().getCargos(idRegimenLaboral, idAreaDesempenioLaboral, idTipoCargo, activo)
        .pipe(
          catchError((error: HttpErrorResponse) => {
            this.dataService.Message().msgWarning(error.error.messages[0]); return of(null);
          }),
          finalize(() => this.descargarLoading())
            ...
            if (response) { this.cargos = response; }
        });
  }
  ...
  setParametrosConsulta() {
    ...
    this.consulta = {
      codigoTipoReporteCAP: CodigoTipoReporteCAP.REPORTE_CAP,
      ...
      idRegimenLaboral: +(data.idRegimenLaboral !== "-1" ? data.idRegimenLaboral : 0),
      idAreaDesempenioLaboral: +(data.idAreaDesempenioLaboral !== "-1" ? data.idAreaDesempenioLaboral : 0),
      idTipoCargo: +(data.idTipoCargo !== "-1" ? data.idTipoCargo : 0),
      idCargo: +(data.idCargo !== "-1" ? data.idCargo : 0),
    ...
    this.dataService.ReportesCAP().GenerarReporteCAP(this.consulta)
      ...
      .subscribe(codigoDocumentoGenerado => {
        if (codigoDocumentoGenerado) {
          ...
          //this.handlePreview("Cuadro de Asignación de Personal", this.pdfFromBase64(codigoDocumentoGenerado.cadenaBase64), "Cuadro de Asignación de Personal");
        }
      });
  }
  ...
  handleExportar() {
    if (this.consulta) {
      this.cargarLoading();
      this.dataService.ReportesCAP().exportarCuadroAsignacionPersonal(this.consulta).pipe(
        catchError((error: HttpErrorResponse) => {
          this.dataService.Message().msgWarning(error.error.messages[0]); return of(null);
        }),
        finalize(() => { this.descargarLoading(); })
        ...
        if (response) {
          ...
          descargarExcel(response, `Reporte CAP ${anio} ${subinstancia} ${tipoCentroTrabajo}.xlsx`);
        }
      });
    } else {
      this.dataService.Message().msgWarning(MESSAGE_REPORTES_CAP.M01, () => { });
  ...
  handleVerReporteCAP(codigoDocumentoGenerado: string) {
    ...
    this.dataService.Documento().descargar(codigoDocumentoGenerado)
      .pipe(
        ...
        if (response) {
          this.handlePreview("Cuadro de Asignación de Personal", response, "Cuadro de Asignación de Personal");
        }
      });
  }
...
export class ReporteDataSource extends DataSource<any>{
  ...
  load(data: any, pageIndex: number, pageSize: number, autoSearch: boolean) {
    this._loadingChange.next(true);
    this.dataService.Spinner().show("sp6");
    this.dataService.ReportesCAP().consultarCuadroAsignacionPersonal(data, pageIndex, pageSize).pipe(
      catchError((error: HttpErrorResponse) => {
        if (!autoSearch){
          this._totalRows = 0;
