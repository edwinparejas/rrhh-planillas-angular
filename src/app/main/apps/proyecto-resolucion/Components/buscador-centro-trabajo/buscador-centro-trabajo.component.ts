...
import { CentroTrabajoEnum, TablaRolPassport , TablaEquivalenciaSede} from '../../_utils/constants';
import { SecurityModel } from 'app/core/model/security/security.model';

@Component({
  selector: "minedu-proyresolucion-buscador-centro-trabajo",
  templateUrl: "./buscador-centro-trabajo.component.html",
  styleUrls: ["./buscador-centro-trabajo.component.scss"],
...
export class BuscarCentroTrabajoComponent
  ...
  configurarInstanciaSubInstancia = () => {
    ...
    if (
      ...
      && this.currentSession.codigoTipoSede == TablaEquivalenciaSede.codigoTipoSedeDre
    ) {
    ...
    if (
      ...
      && this.currentSession.codigoTipoSede == TablaEquivalenciaSede.codigoTipoSedeUgel
    ) {
      this.desactivarIntancia = false;
  ...
  obtenerInstanciaSubinstanciaDefault() {
    ...
    if (this.data.codigoSede != null) {
      this.dataService.ProyectosResolucion().getDefaultValueComboInstanciasByCodSede(this.data.codigoSede).pipe(
        catchError(() => of([])),
        finalize(() => { })
      )
      ...
      if (value === "-1") {
        switch (this.passport.idNivelInstancia) {
          case TablaNivelInstancia.MINEDU: {
            ...
            if (codigoTipoSede == TablaEquivalenciaSede.codigoTipoSedeDre && this.form.get("idSubinstancia").value == "-1") {
              this.mostrarOpciones(true, true, false, false);
              break;
            }
            this.mostrarOpciones(true, true, true, false);
...
debugger
  ...
  loadTipoCentroTrabajo = (idNivelInstancia, activo) => {
    this.dataService
      .ProyectosResolucion()
      .listarTipoCentroTrabajo(idNivelInstancia, activo)
      .pipe(
        catchError(() => of([])),
        map((response: any) => response)
      )
  ...
  limpiar = () => {
    let codigoTipoSede = this.currentSession.codigoTipoSede;
    if (codigoTipoSede == TablaEquivalenciaSede.codigoTipoSedeOficina) {
      this.resetFiltro();
      this.mostrarModalidadNivelEducativo = false;
    } else if (codigoTipoSede == TablaEquivalenciaSede.codigoTipoSedeDre) {
      this.ocultarTipoCentroTrabajo = false;
...
export class CentroTrabajoDataSource extends DataSource<any> {
  ...
  load = (filtro: any, pageIndex, pageSize) => {
    ...
    this.dataService
      .ProyectosResolucion()
      .listarCentroTrabajo(filtro, pageIndex, pageSize)
      .pipe(
        catchError(() => of([])),
        finalize(() => {
          this._loadingChange.next(false)
