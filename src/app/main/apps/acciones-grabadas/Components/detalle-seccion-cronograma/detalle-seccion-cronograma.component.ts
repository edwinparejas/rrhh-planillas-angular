...
export class DetalleSeccionCronogramaComponent implements OnInit {
    ...
    dataObject:{id:number,valorCampo:string,colspan:number,bold:number,centrado:number,border:number}[]=[];
    ...
    ngOnInit(): void {    
        ...
        this.detalleVistaProyecto.forEach(async (detalle) => {        
            var splitg=JSON.parse(detalle.valorTexto);
            splitg.forEach(async (split) => {
                if(split.length==1){
                    this.dataObject.push({id:cantidad,valorCampo:split[0],colspan:5,bold:1,centrado:1,border:1}); 
                    ...
                    split.forEach(async (rowFilter) => {
                        ...
                        this.dataObject.push({id:cantidad,valorCampo:rowFilter,colspan:1,bold:bold,centrado:0,border:1}); 
                        if(this.saltoLinea){
                            this.dataTable.push({id:apoyo,dataObject:this.dataObject});
                            this.dataObject=[];
                            apoyo=apoyo+1;
