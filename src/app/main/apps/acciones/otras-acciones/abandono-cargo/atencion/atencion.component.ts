...
export class AtencionComponent implements OnInit {
        ...
        (response) => {
          if(response)
          {
            this.form.patchValue({
              ...
              // conPlazoEspejo: response.,
              // codigoPlazoEspejo: response.,
              // itemPlazaEspejo: response.,
  ...
  loadComboMotivoAccion() {
    ...
    this.dataService.OtrasFuncionalidades().getMotivoAccion(request).pipe(
      ...
      finalize(() => {
          this.dataService.Spinner().hide('sp6');          
          this.setLabelFechaReporte(this.form.get('idMotivoAccion').value);
      })
      ).subscribe(
        (response) => {
  ...
  onChangeRegimenLaboral(event: any){
    this.loadComboMotivoAccion();
  }
  ...
  buscarServidorPublico(){
    ...
    this.dataService.OtrasFuncionalidades().getServidorPublicoTransversal(this.datosAtencionReporte).pipe(
      catchError((error: HttpErrorResponse) => {
        this.dataService.Util().msgWarning(error.error.messages[0]);
        return of(null);
      }),
      ...
      (response) => {
        if(response && (response || []).length > 0)
        {
          ...
          if (this.idAtencionReporte > 0) {
            ...
            this.datosLaborales.itemPlaza = this.form.get('itemPlaza').value;
            ...
            this.form.patchValue({ 
              plazaDetalleAtencion: new PlazaDetalleAtencion(this.form.get('idPlazaDetalleReporte').value, this.idAtencionReporte, this.datosLaborales.codigoPlaza, this.datosLaborales.itemPlaza
                                    ...
                                    , this.datosLaborales.idCategoriaRemunerativa, this.datosLaborales.idTipoCargo, this.datosLaborales.idCargo, this.datosLaborales.idTipoPlaza, this.datosLaborales.idEstadoPlaza, 0),
            ...
            this.form.patchValue({ 
              ...
              itemPlaza: this.datosLaborales.idPlaza, 
  ...
  GetFormData(): any {
    ...
    formDataReporte.append('idReporte', formRequest.idReporte);
    ...
    formDataReporte.append('idEstadoAtencion', formRequest.idEstadoAtencion);
    if(formRequest.idReporteRelacionado)
      formDataReporte.append('idReporteRelacionado', formRequest.idReporteRelacionado);  
    if (this.idAtencionReporte == 0) {
    ...
    formDataReporte.append('plazaDetalleAtencion.idReporte', formRequest.idReporte);
    ...
    formDataReporte.append('servidorPublicoDetalleAtencion.idReporte', formRequest.idReporte);
    ...
    for (let i = 0; i < formRequest.documentoSustento.length; i++) {
      ...
      formDataReporte.append('documentoSustento[' + i + '].idReporte', formRequest.documentoSustento[i].idReporte);
      ...
      if (this.idAtencionReporte == 0) {
        this.dataService.OtrasFuncionalidades().postAtencionReporte(this.GetFormData()).pipe(
          ...
          (response) => {
            if(response || (response || []).length == 0)
            {
              this.form.get('idReporte').setValue(response);
              if (!this.isSend) {
  ...
  handleAgregarSustento(){
    ...
    this.form.get('sustentatorio.idReporte').setValue(this.form.get('idReporte').value);
    this.form.get('sustentatorio.fechaRegistro').setValue(this.now);
