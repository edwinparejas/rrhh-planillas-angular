...
@Component({
    selector: 'minedu-datos-sancion-info',
    templateUrl: './datos-sancion-info.component.html',
    styleUrls: ['./datos-sancion-info.component.scss']
})
export class DatosSancionInfoComponent implements OnInit, OnDestroy {
    @Input('sancion')
    sancion: any;
    ...
    private async cargarAccionPersonal() {
        ...
        let idTipoCargo = this.sancion?.idTipoCargo ?? this.actividadResolucion?.idTipoCargoNuevo;
        let idCargo = this.sancion?.idCargo ?? this.actividadResolucion?.idCargoNuevo;
        let idJornadaLaboral = this.sancion?.idJornadaLaboral ?? this.actividadResolucion?.idJornadaNuevo;
