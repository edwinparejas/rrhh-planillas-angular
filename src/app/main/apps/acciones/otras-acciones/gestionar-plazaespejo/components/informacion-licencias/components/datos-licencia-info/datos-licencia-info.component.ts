...
@Component({
    selector: 'minedu-datos-licencia-info',
    templateUrl: './datos-licencia-info.component.html',
    styleUrls: ['./datos-licencia-info.component.scss']
})
export class DatosLicenciaInfoComponent implements OnInit, OnDestroy {
    @Input('licencia')
    licencia: any;
    ...
    private async cargarAccionPersonal() {
        ...
        let idTipoCargo = this.licencia?.idTipoCargo ?? this.actividadResolucion?.idTipoCargoNuevo;
        let idCargo = this.licencia?.idCargo ?? this.actividadResolucion?.idCargoNuevo;
        let idJornadaLaboral = this.licencia?.idJornadaLaboral ?? this.actividadResolucion?.idJornadaNuevo;
