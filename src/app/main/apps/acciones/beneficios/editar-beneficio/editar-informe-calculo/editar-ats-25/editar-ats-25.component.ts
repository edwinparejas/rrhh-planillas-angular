...
@Component({
  selector: 'minedu-editar-ats-25',
  templateUrl: './editar-ats-25.component.html',
  styleUrls: ['./editar-ats-25.component.scss'],
  encapsulation: ViewEncapsulation.None,
    animations: mineduAnimations
})
export class EditarAts25Component implements OnInit {
    ...
    ngOnInit() {
        ...
        this.form.get("guardar").valueChanges.subscribe(x => {
         ...
         this.form.get("validarGenerarProyecto").valueChanges.subscribe(x => {
            if(x && this.form.get('tipoFormularioBeneficio').value==TipoFormularioBeneficioEnum.ATS25){
    ...
    * Métodos Integración Back*/
     ...
     guardar(){
        ...
        this.dataService.Message().msgConfirm('¿ESTÁ SEGURO QUE DESEA GUARDAR LA INFORMACIÓN?', () => {
            ...
            this.dataService.Beneficios().guardarEditarBeneficioATS25(generarFormDataUtil(this.form.value)).pipe(
                catchError((response : HttpErrorResponse) =>{
                    this.dataService.Message().msgWarning(response.error.messages[0]);
                    return of(null);
                }),
