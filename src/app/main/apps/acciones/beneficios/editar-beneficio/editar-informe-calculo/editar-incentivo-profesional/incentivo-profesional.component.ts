...
@Component({
  selector: 'minedu-editar-incentivo-profesional',
  templateUrl: './editar-incentivo-profesional.component.html',
  styleUrls: ['./editar-incentivo-profesional.component.scss'],
  encapsulation: ViewEncapsulation.None,
    animations: mineduAnimations
})
export class EditarIncentivoProfesionalComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.form.get("guardar").valueChanges.subscribe(x => {
         ...
         this.form.get("validarGenerarProyecto").valueChanges.subscribe(x => {
            if(x && this.form.get('tipoFormularioBeneficio').value==TipoFormularioBeneficioEnum.IncentivoProfesional){
...
/**
 ...
 loadCombos() {
    ...
    this.loadTipoBeneficio();
    this.loadCategoriaBeneficio();
};
loadTipoBeneficio() {
    this.dataService.Beneficios().getComboTipoBeneficio().pipe(catchError(() => of([])), finalize(() => { })).subscribe((result: any) => {
        if (result) {
            if(result.length == 0)
            {
                this.dataService.Message().msgInfo(
                    "NO HAY REGISTROS DE TIPO BENEFICIO", () => {
                    
                });
            }
            this.comboLists.listTipoBeneficio = result.map((x) => ({
                ...x,
                value: x.idTipoBeneficio,
                label: x.descripcionTipoBeneficio
            }));
        }
        
    });
}
loadCategoriaBeneficio() {
    this.dataService.Beneficios().getComboCategoriaBeneficio().pipe(catchError(() => of([])), finalize(() => { })).subscribe((result: any) => {
        if (result) {
            if(result.length == 0)
            {
                this.dataService.Message().msgInfo(
                    "NO HAY REGISTROS DE CATEGORÍA", () => {
                    
                });
            }
            this.comboLists.listCategoria = result.map((x) => ({
                ...x,
                value: x.idCategoriaBeneficio,
                label: x.descripcionCategoriaBeneficio
            }));
        }
  ...
  selectConInformeCalculo(){
    ...
    * Métodos Integración Back*/
     ...
     validarGuardar(){
        ...
        if(!this.form.get('idTipoBeneficio').value)
        {
        ...
        if(!this.form.get('idCategoriaBeneficio').value)
        {
            this.dataService.Message().msgAutoCloseWarning(MensajesSolicitud.M20, 3000, () => {
     ...
     guardar(){
        ...
        this.dataService.Message().msgConfirm('¿ESTÁ SEGURO QUE DESEA GUARDAR LA INFORMACIÓN?', () => {
            ...
            this.dataService.Beneficios().guardarEditarBeneficioIncentivoProfesional(generarFormDataUtil(this.form.value)).pipe(
                catchError((response : HttpErrorResponse) =>{
                    this.dataService.Message().msgWarning(response.error.messages[0]);
                    return of(null);
                }),
    ...
    * Métodos Util */
      ...
      getData() {
        ...
        idTipoBeneficio:dataForm.idTipoBeneficio,
        idCategoriaBeneficio:dataForm.idCategoriaBeneficio,
