...
import { of } from 'rxjs';
import { catchError, finalize, pairwise, startWith } from 'rxjs/operators';
...
@Component({
  selector: 'minedu-editar-cts-29944',
  templateUrl: './editar-cts-29944.component.html',
  styleUrls: ['./editar-cts-29944.component.scss'],
  encapsulation: ViewEncapsulation.None,
    animations: mineduAnimations
})
export class Editarcts29944Component implements OnInit {
    ...
    ngOnInit() {
        this.form.get("aniosTiempoServicio").valueChanges.pipe(startWith(null), pairwise()).subscribe(([prev, next]: [any, any]) => {
            if(next)
                this.loadImporteBeneficio();
         })
         this.form.get("baseCalculo").valueChanges.pipe(startWith(null), pairwise()).subscribe(([prev, next]: [any, any]) => {
            if(next)
                this.loadImporteBeneficio();
         })
        this.form.get("guardar").valueChanges.subscribe(x => {
         ...
         this.form.get("validarGenerarProyecto").valueChanges.subscribe(x => {
            if(x && this.form.get('tipoFormularioBeneficio').value==TipoFormularioBeneficioEnum.CTS29944){
  ...
  selectConInformeCalculo(){
    ...
    * Métodos Integración Back*/
     EnviarAccionesGrabadas =()=>{
        ...
        this.dataService.Message().msgConfirm(mensajeEnviar,() => {
    ...
    validarGuardar(){
     ...
     guardar(){
        ...
        this.dataService.Message().msgConfirm('¿ESTÁ SEGURO QUE DESEA GUARDAR LA INFORMACIÓN?', () => {
            ...
            this.dataService.Beneficios().guardarEditarBeneficioCTS29944(generarFormDataUtil(this.form.value)).pipe(
                catchError((response : HttpErrorResponse) =>{
                    this.dataService.Message().msgWarning(response.error.messages[0]);
                    return of(null);
                }),
