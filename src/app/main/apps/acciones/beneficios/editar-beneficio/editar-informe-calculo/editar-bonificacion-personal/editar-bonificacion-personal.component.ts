...
@Component({
  selector: 'minedu-editar-bonificacion-personal',
  templateUrl: './editar-bonificacion-personal.component.html',
  styleUrls: ['./editar-bonificacion-personal.component.scss'],
  encapsulation: ViewEncapsulation.None,
    animations: mineduAnimations
})
export class EditarBonificacionPersonalComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        this.form.get("guardar").valueChanges.subscribe(x => {
         ...
         this.form.get("validarGenerarProyecto").valueChanges.subscribe(x => {
            if(x && this.form.get('tipoFormularioBeneficio').value==TipoFormularioBeneficioEnum.BonificacionPersonal){
    ...
    * Métodos Integración Back*/
     ...
     guardar(){
        ...
        this.dataService.Message().msgConfirm('¿ESTÁ SEGURO QUE DESEA GUARDAR LA INFORMACIÓN?', () => {
            ...
            this.dataService.Beneficios().guardarEditarBeneficioBonificacionPersonal(generarFormDataUtil(this.form.value)).pipe(
                catchError((response : HttpErrorResponse) =>{
                    this.dataService.Message().msgWarning(response.error.messages[0]);
                    return of(null);
                }),
     ...
     getQuinquenioTexto(val){
      ...
      loadImporteBeneficio(){
        ...
        if(this.form.get("aniosTiempoServicio").value!=null){
            ...
            let quinquenio = Math.floor(tiempoServicio/5);
            if(quinquenio==0){
                this.dataService.Message().msgAutoCloseWarning('"SE REQUERIE 5 AÑOS A MÁS PARA EL BENEFICIO."', 3000, () => {
                    this.form.patchValue({ 
                        importeBeneficio : 0
                        });
                });
            }
            let quinquenioTexto = this.getQuinquenioTexto(quinquenio);
            ...
            let importeBeneficio = baseCalculo*factorCalculoformula*quinquenio;
      ...
      getData() {
        ...
        quinquenio:dataForm.quinquenio,
