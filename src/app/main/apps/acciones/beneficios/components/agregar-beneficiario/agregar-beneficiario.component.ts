...
@Component({
  ...
  styleUrls: ['./agregar-beneficiario.component.scss'],
...
export class AgregarBeneficiarioComponent implements OnInit {
    ...
    constructor(
        public matDialogRef: MatDialogRef<AgregarBeneficiarioComponent>,
    ...
    handleValidarReniec(val){
        ...
        this.dataService.Beneficios().validarDNI(queryParam).pipe(catchError(() => of([])), finalize(() => { this.dataService.Spinner().hide('sp6'); })).subscribe((response: any) => {
            if (response) {
                this.formPopup.patchValue({ 
                    idEstadoReniec: 1,
                    estadoRENIEC: "ACTIVO",
