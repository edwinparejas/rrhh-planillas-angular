...
import { MESSAGE_GESTION } from '../../models/desvinculacion.model';
...
export class GenerarProyectoComponent implements OnInit, AfterViewInit {
  ...
  handleGenerarProyecto = () => {
    ...
    this.dataService.Message().msgConfirm('¿ESTÁ SEGURO QUE DESEA GENERAR EL PROYECTO?', () => { 
      ...
      if (this.data.idGestionDesvinculacion != null && this.data.idGestionDesvinculacion > 0) {
       
        this.dataService.Spinner().show("sp6");
        data.id_gestion_desvinculacion = this.data.idGestionDesvinculacion;
      ...
      return;
        ...
        //------------------- SE EJECUTA CUANDO SE DA CLICK AL CREAR LA DESVINCULACION ------------------------
      this.dataService.AccionesDesvinculacion().guardarGestionDesinculacion(this.data.formDataVinculacion).subscribe(
        (responseIdVinculacion) => {
          this.dataService.Spinner().show("sp6");
          debugger 
          data.id_gestion_desvinculacion = parseInt(responseIdVinculacion.toString())
  ...
  GenerarProyecto(data){
    ...
    const formData = new FormData();
        ...
        this.dataService.AccionesDesvinculacion()
          .generarProyectoResolucionDesvinculacion(formData)
          .pipe(
            catchError((error) => {
          ...
          .subscribe((response: any) => {
              ...
              if (response != null) {                  
                    ...
                    this.dataService.Message().msgAutoCloseSuccessNoButton('"PROYECTO GENERADO CORRECTAMENTE"', 3000, () => {
                      this.matDialogRef.close({ id_gestion_desvinculacion: response});
