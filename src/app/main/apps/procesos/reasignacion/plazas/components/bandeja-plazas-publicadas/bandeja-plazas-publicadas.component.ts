...
@Component({
  selector: 'minedu-bandeja-plazas-publicadas',
  templateUrl: './bandeja-plazas-publicadas.component.html',
  styleUrls: ['./bandeja-plazas-publicadas.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: mineduAnimations,
})
export class BandejaPlazasPublicadasComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    handleExportarPlazasPublicadas = () => {
        ...
        this.dataService
            .Reasignaciones()
            .exportarPlazaReasignacionPublicadas(this.request)
            .pipe(
                catchError((e) => of(null)),
                finalize(() => {
                    this.dataService.Spinner().hide('sp6');
            ...
            .subscribe((response: any) => {
                if (response) {
                    descargarExcel(response, 'plazas_publicadas.xlsx');
                } else {
                    this.dataService
                        .Message()
                        .msgWarning(
...
export class PrepublicadasDataSource extends DataSource<any> {
    ...
    load(data: any, pageIndex, pageSize, selectedTabIndex: number): void {
        this.dataService.Reasignaciones()
            .getGridPlazaReasignacionPublicadas(data, pageIndex, pageSize)
            .pipe(
                catchError(() => of(null)),
                finalize(() => {
    ...
    private gridResultMessage = (selectedTabIndex: number, emptyResult: boolean = true) => {
        ...
        if (emptyResult) {
            if(selectedTabIndex == 1){
                this.dataService.Message()
                .msgWarning('"NO SE ENCONTRÓ INFORMACIÓN DE PLAZAS PUBLICADAS PARA LOS CRITERIOS DE BÚSQUEDA INGRESADOS."', () => {});
            }
        } else {
            this.dataService.Message()
            .msgWarning('"OCURRIERON PROBLEMAS AL MOMENTO DE BUSCAR PLAZAS PUBLICADAS PARA LOS CRITERIOS DE BÚSQUEDA INGRESADOS."', () => { });
        }
    }
