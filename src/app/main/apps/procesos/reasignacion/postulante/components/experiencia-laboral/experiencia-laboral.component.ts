...
import { ExperienciaLaboralModel } from '../../../models/reasignacion.model';
...
export class ExperienciaLaboralComponent implements OnInit {
    ...
    loadTipoExperiencia = () => {
        this.dataService.Reasignaciones().getComboTipoExperiencia().pipe(
            catchError(() => of([])),
            finalize(() => { })
    ...
    loadNivelExperiencia = () => {
        this.dataService.Reasignaciones().getComboNivelExperiencia().pipe(
            catchError(() => of([])),
            finalize(() => { })
        ).subscribe((response: any) => {
            if (response && response.result) {
    ...
    loadTipEntidad = () => {
        this.dataService.Reasignaciones().getComboTipoEntidad().pipe(
            catchError(() => of([])),
            finalize(() => { })
        ).subscribe((response: any) => {
            if (response && response.result) {
