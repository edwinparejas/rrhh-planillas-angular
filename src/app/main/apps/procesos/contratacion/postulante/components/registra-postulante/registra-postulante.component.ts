...
import {
    PostulacionModel,
    ServidorPublicoModel,
    DocumentoSustentoModel,
    DatoPlazaPostularModel,
    FormacionAcademicaModel,
    CapacitacionModel,
    ExperienciaLaboralModel,
    ExperienciaLaboralCalc,
    ResumenPlazasResponseModel,
    PlazaGrillaModel,
    PlazaPostuladaModel,
} from "../../../models/contratacion.model";
import {
    CodigoTipoOperacionSustentoEnum,
    OrigenRegistroEnum,
...
export class RegistraPostulanteComponent implements OnInit {
    ...
    @ViewChild(FormacionAcademicaComponent)
    private formacionAcademicaComponent: FormacionAcademicaComponent;
    @ViewChild(CapacitacionesComponent)
    private capacitacionesComponent: CapacitacionesComponent;
    @ViewChild(ExperienciaLaboralComponent)
    private experienciaLaboralComponent: ExperienciaLaboralComponent;
    ...
    buildForm = () => {
        this.form = this.formBuilder.group({
            ...
            idEtapa: null,
            idPersona: null,
            idTipoVia: [null, Validators.required],
            ...
            numeroVia: [null, Validators.required],
            direccion: null,
    ...
    loadTipoSustento = () => {
        this.dataService
            ...
            .getComboTipoSustento(
                CodigoTipoOperacionSustentoEnum.OTROS_DATOS_POSTULACION
            )
            .pipe(
                catchError(() => of([])),
                finalize(() => {})
    ...
    handleSave = (form) => {
        if (!this.form.valid) {
            this.dataService
                .Message()
                .msgWarning("Debe ingresar datos obligatorios.", () => {});
            return;
        }
        ...
        if (this.formacionesAcademica.length === 0) {
            this.dataService
                .Message()
                .msgWarning("Debe ingresar formación académica.", () => {});
            return;
        }
        ...
        if (this.capacitaciones.length === 0) {
            this.dataService
                .Message()
                .msgWarning("Debe ingresar capacitación.", () => {});
            return;
        }
        ...
        if (this.experienciasLaborales.length === 0) {
            this.dataService
                .Message()
                .msgWarning("Debe ingresar experiencia laboral.", () => {});
            return;
        }
        ...
        if (this.idOperacion === TipoOperacionEnum.Registrar) {
            this.dataService
                .Message()
                .msgConfirm(
                    "¿Esta seguro de que desea guardar la información?",
                    () => {
    ...
    modificarPostulacion = (postulacion: any) => {
        ...
        this.dataService
            .Contrataciones()
            .modificarPostulacion(postulacion)
            .pipe(
                catchError((e) => of(e)),
                finalize(() => {
                    this.dataService.Spinner().hide("sp6");
    ...
    prepararData = (
        row: any,
        idOperacion: number = TipoOperacionEnum.Registrar
    ) => {
        const model: PostulacionModel = new PostulacionModel();
        model.idPostulacion =
            this.idOperacion === TipoOperacionEnum.Registrar
                ? 0
                : this.postulacion.idPostulacion;
        ...
        model.idColegioProfesional =
            row.idColegioProfesional == -1 ? null : row.idColegioProfesional;
        ...
        if (row.idCargo != null) {
            ...
            model.datoPlazaPostular.idModalidadEducativa =
                row.idModalidadEducativa;
    ...
    loadProvincia = (idDepartamento) => {
        ...
        this.dataService
            .Contrataciones()
            .getComboProvincia(idDepartamento)
            .pipe(
                catchError(() => of([])),
                finalize(() => {})
            )
    ...
    loadDistrito = () => {
        ...
        this.dataService
            .Contrataciones()
            .getComboDistrito(idDepartamento, idProvincia)
            .pipe(
                catchError(() => of([])),
                finalize(() => {})
            )
    ...
    buscarPersona(): void {
        ...
        if (
            ...
            this.dataService
                .Message()
                .msgWarning(
                    "Debe ingresar Tipo de documento y Número de documento",
                    () => {}
                );
            return;
        }
            ...
            .subscribe((response: any) => {
                if (response && response.result) {
                    ...
                    if (
                        ...
                        this.form
                            .get("idPersona")
                            .setValue(this.persona.idPersona);
                    }
                }
            });
    }
    ...
    bindData = (row: PostulacionModel) => {
        ...
        this.form
            ...
            .setValue(row.idTipoDocumentoIdentidad);
        this.form
            .get("numeroDocumentoIdentidad")
            .setValue(row.numeroDocumentoIdentidad);
        this.form.get("numeroRuc").setValue(row.numeroRuc);
        this.form
            .get("idColegioProfesional")
            .setValue(row.idColegioProfesional);
        this.form
            .get("numeroColegioProfesional")
            .setValue(row.numeroColegioProfesional);

        if (row.datoPlazaPostular != null) {
            this.form.get("idCargo").setValue(row.datoPlazaPostular.idCargo);
            this.form
                .get("idModalidadEducativa")
                .setValue(row.datoPlazaPostular.idModalidadEducativa);
            this.form
                .get("idNivelEducativo")
                .setValue(row.datoPlazaPostular.idNivelEducativo);
            this.form
                .get("idAreaCurricular")
                .setValue(row.datoPlazaPostular.idAreaCurricular);
        }
        ...
        (this.persona.idPersona = row.idPersona),
            (this.persona.idTipoDocumentoIdentidad =
                row.idTipoDocumentoIdentidad);
        this.persona.numeroDocumentoIdentidad = row.numeroDocumentoIdentidad;
        this.persona.numeroDocumentoIdentidadCompleto =
            row.numeroDocumentoIdentidadCompleto;
        ...
        this.formacionAcademicaComponent.actualizarLista(
            this.formacionesAcademica
        );
        this.capacitacionesComponent.actualizarLista(this.capacitaciones);
        this.experienciaLaboralComponent.actualizarLista(
        ...
        if (row.plazaPostulada != null) {
            ...
            this.form
                .get("codigoPlaza")
                .setValue(row.plazaPostulada.codigoPlaza.trim());
            this.plaza = new PlazaGrillaModel();
            this.form
                .get("idPlazaPostulada")
                .setValue(row.plazaPostulada.idPlazaPostulada);
            this.form
                .get("idPlazaContratacion")
                .setValue(row.plazaPostulada.idPlazaContratacion);
            this.plaza.codigoModular = row.plazaPostulada.codigoModular;
            this.plaza.descripcionInstitucionEducativa =
                row.plazaPostulada.descripcionInstitucionEducativa;
            this.plaza.abreviaturaModalidadEducativa =
                row.plazaPostulada.abreviaturaModalidadEducativa;
            this.plaza.descripcionNivelEducativo =
                row.plazaPostulada.descripcionNivelEducativo;
            this.plaza.descripcionTipoGestionEducativa =
                row.plazaPostulada.descripcionTipoGestionEducativa;
            ...
            this.plaza.descripcionAreaCurricular =
                row.plazaPostulada.descripcionAreaCurricular;
            this.plaza.jornadaLaboral = row.plazaPostulada.jornadaLaboral;
            this.plaza.descripcionTipoPlaza =
                row.plazaPostulada.descripcionTipoPlaza;
            this.plaza.descripcionMotivoVacancia =
                row.plazaPostulada.descripcionMotivoVacancia;
            this.plaza.codigoCentroTrabajo =
                row.plazaPostulada.codigoCentroTrabajo;
            this.plaza.idPlazaContratacion =
                row.plazaPostulada.idPlazaContratacion;
            this.plaza.descripcionRegimenLaboral =
                row.plazaPostulada.descripcionRegimenLaboral;
            this.plaza.idGrupoInscripcion =
                row.plazaPostulada.idGrupoInscripcion;
            this.plaza.descripcionGrupoInscripcion =
                row.plazaPostulada.descripcionGrupoInscripcion;
        }
    ...
    buscarPlaza = () => {
        ...
        this.dataService
            .Contrataciones()
            .buscarPlazasPaginado(data, 1, 20)
            .pipe(
                catchError(() => of([])),
            ...
            .subscribe((response: any) => {
                if ((response.data || []).length === 0) {
                    ...
                    this.form
                        .get("idPlazaContratacion")
                        .setValue(this.plaza.idPlazaContratacion);
                }
            });
    };
    ...
    busquedaPlazaPersonalizada(): void {
        ...
        this.dialogRef.afterClosed().subscribe((resp) => {
            if (resp != null) {
                ...
                this.form
                    .get("idPlazaContratacion")
                    .setValue(plaza.idPlazaContratacion);
                this.plaza = plaza;
            }
        });
    }
    ...
    configurarRegistro(): void {
        if (
            this.etapaResponse.codigoRegimenLaboral ===
                this.regimenLaboral.LEY_30512 &&
            (this.etapaResponse.codigoEtapaFase === this.etapaFaseEnum.FASE2 ||
                this.etapaResponse.codigoEtapaFase === this.etapaFaseEnum.FASE3)
        ) {
            this.form
                .get("idPlazaContratacion")
