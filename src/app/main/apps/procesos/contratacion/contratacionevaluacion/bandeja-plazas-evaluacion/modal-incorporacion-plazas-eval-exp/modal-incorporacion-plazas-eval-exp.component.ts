...
@Component({
    selector: 'minedu-modal-incorporacion-plazas-eval-exp',
    templateUrl: './modal-incorporacion-plazas-eval-exp.component.html',
    styleUrls: ['./modal-incorporacion-plazas-eval-exp.component.scss'],
    encapsulation: ViewEncapsulation.None,
    animations: mineduAnimations,
})
export class ModalIncorporacionPlazasEvalExpComponent implements OnInit {
    form: FormGroup;
    dialogRef: any;
    estadoPlaza = 0;
    becario: string;
    ...
    constructor(
        public matDialogRef: MatDialogRef<ModalIncorporacionPlazasEvalExpComponent>,
    ...
    handleIncorporarPlazas = () => {
        ...
        this.dataService.Message().msgConfirm('¿ESTÁ SEGURO QUE DESEA INCORPORAR LAS PLAZAS PARA CONTRACIÓN DE RESULTADOS pun?',
            () => {
                ...
                this.dataService.Contrataciones().registrarPlazasIncorporarContratacionEvalExp(request).pipe(catchError((e) => of(null)),
                    finalize(() => {
...
export class PlazasContratacionPlazasGeneradasDataSource extends DataSource<any> {
    ...
    load(data: any, pageIndex, pageSize): void {
        ...
        if (data.idEtapaProceso === null) {
            ...
            this.dataService.Contrataciones().postBuscarPlazasPublicadasPaginadoEvalExp(data, pageIndex, pageSize).pipe(
                catchError(() => of([])),
                finalize(() => {
                    this._loadingChange.next(false);
                    this.dataService.Spinner().hide("sp6");
