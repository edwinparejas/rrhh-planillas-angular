...
@Component({
  selector: 'minedu-registrar-calificaciones-eval',
  templateUrl: './registrar-calificaciones-eval.component.html',
  styleUrls: ['./registrar-calificaciones-eval.component.scss'],
  animations: mineduAnimations,
    encapsulation: ViewEncapsulation.None,
})
export class RegistrarCalificacionesEvalComponent implements OnInit {
    ...
    preModalidadEducativa:any;
    preNivelEducativo:any;
    preAreaCurricular:any;
    preEspecialidad:any;
    preFechaExpedicionTitulo:any;
    preTitulo:any;
    ...
    obtenerPrelacion(){
        ...
        this.dataService.Contrataciones().getBuscarMeritoEvalExpPaginadoPorId(d).pipe(catchError(() => of([])),
            finalize(() => {
                this.working = false;
            })
        )
        .subscribe((response: any) => {
            // console.log("ObtenerInformacion:getObtenerCalificacion: ", response)
            if (response[0] != null) {
                console.log('Response',  response[0])
                this.preModalidadEducativa = response[0].modalidadEducativa;
                this.preNivelEducativo = response[0].nivelEducativo;
                this.preAreaCurricular = response[0].areaCurricular;
                this.preEspecialidad = response[0].especialidad;
                this.preFechaExpedicionTitulo = response[0].fechaTitulo
                this.preTitulo = response[0].titulo;
            }
    ...
    verificarInicioCumpleCheck(){ // editar calificaiones
            ...
            if(rub.idTipoPuntaje == 1){ // tipo pubtaje = cumpledocumento
                ...
                this.dtSource[contador].data.forEach(d => {
                    if(d.cumpleDocumento == false){
                        verificarActivarDetalleCalificacionCompleto = false;
                        totalFalsos = totalFalsos +1;
                    }
                });
            }    
            contador++;
           /* if(rub.idTipoPuntaje == 1){
                if(this.dtSource[contador].data.length > 0){
                    this.dtSource[contador].data.forEach(d => {
                        if(d.cumpleDocumento == false){
                            verificarActivarDetalleCalificacionCompleto = false;
        ...
        console.log('TotalDatos / Total False', totalFalsos, totalDatos)
    ...
    changeCumpleCheck(event, dtIndex, rowIndex) {
        ...
        // verificar activacion de campos en el formulario
        if ( this.rubro[dtIndex] != null && this.rubro[dtIndex].idTipoPuntaje  == 1){ // !this.activarDetalleCalificacionCompleto ||
            var contador = 0;
            var verificarActivarDetalleCalificacionCompleto = true; // variable temporal para determinar sis e cumplen todos los requisitos
            this.rubro.forEach(rub =>{
                if(rub.idTipoPuntaje == 1){
    ...
    calcularPuntajeObtenidoTotal(row, dtIndex, rowIndex) {
        ...
        if (this.dtSource[dtIndex].data[rowIndex].cumpleDocumento) {
            if (row.tieneCantidad) {
                ...
                console.log("CalcularPuntajeObtenidoTotal: ", row)
                if (!row.tieneSubCriterio && (row.idTipoPuntaje != TipoPuntajeEnum.BONIFICACIONES)){
                    puntajeObtenido = this.dtSource[dtIndex].data[rowIndex].puntajeMaximo;
                }
                if (!row.tieneSubCriterio && (row.idTipoPuntaje == TipoPuntajeEnum.BONIFICACIONES)){
                    puntajeObtenido = this.dtSource[dtIndex].data[rowIndex].puntajeMaximo * this.info.puntajeFinal/100;
                }
                puntajeObtenidoText = puntajeObtenido.toFixed(2);
    ...
    CalcularPuntajeObtenido(rubro, dtIndex) {
        let puntajeObtenido = 0;
        if (rubro.idTipoPuntaje == TipoPuntajeEnum.PUNTAJE || rubro.idTipoPuntaje == TipoPuntajeEnum.NO_APLICA || rubro.idTipoPuntaje == TipoPuntajeEnum.BONIFICACIONES ) {
    ...
    obtenerInformación = () => {
        ...
        .subscribe((response: any) => {
            ...
            if (response) {
                ...
                this.rubroCab = [];
                console.log("Data INicial Formularios:",response);
                ...
                this.info.rubro.forEach((r, i) => {
                    ...
                    if (r.idTipoPuntaje == TipoPuntajeEnum.BONIFICACIONES) {
                        this.displayedColumns[i].push("puntajeFinalEvaluacion");
                        this.displayedColumns[i].push("porcentajeBonificacion");
                        this.displayedColumns[i].push("sustento");
                        this.displayedColumns[i].push("acreditaDocumento");
                        // this.displayedColumns[i].push("puntajeMaximo");
                        this.displayedColumns[i].push("puntajeObtenido");
                    }
                ...
                console.log("Data Final con load Formularios:",response);
            }
    ...
    handleGuardar = () => {
        this.dataService.Message().msgConfirm(MensajesSolicitud.M02, () => {
            ...
            this.dataService.Contrataciones().postGuardarCalificacion(this.request).pipe(
                ...
                .subscribe((response: any) => {
                    if (response > -1) {
                        this.dataService.Message().msgAutoCloseSuccessNoButton(MensajesSolicitud.M07, 5000,() => {});
...
export class EvaluacionDataSource extends DataSource<any> {
    ...
    load(data: any): void {
        ...
        if (data.idCalificacion === null) {
            ...
            .subscribe((d: any) => {
                console.log("EvaluacionDatasourse: getObtenerCalificacionRubroDetalle: ", d);
