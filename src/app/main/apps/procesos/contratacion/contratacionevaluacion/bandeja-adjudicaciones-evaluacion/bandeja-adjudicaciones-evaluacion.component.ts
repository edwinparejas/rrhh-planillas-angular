...
import { TipoFormatoPlazaEnum, CatalogoItemEnum, EstadoAdjudicacionEnum, MensajesSolicitud } from '../../_utils/constants';
...
import { ModalNoAdjudicarEvalComponent } from "./modal-no-adjudicar-eval/modal-no-adjudicar-eval.component";
...
@Component({
    selector: 'minedu-bandeja-adjudicacion-evaluacion',
    templateUrl: './bandeja-adjudicaciones-evaluacion.component.html',
    styleUrls: ['./bandeja-adjudicaciones-evaluacion.component.scss'],
    animations: mineduAnimations,
    encapsulation: ViewEncapsulation.None,
})
export class BandejaAdjudicacionEvaluacionComponent implements OnInit {
    ...
    loadEstados = () => {
        this.dataService.Contrataciones().getCombosEstadosEtapaEvaluacion(CatalogoItemEnum.ESTADO_CALIFICACION).pipe(
            catchError(() => of([])),
            finalize(() => { })
        ).subscribe((response: any) => {
            if (response) {
    ...
    handleExportar = (dataSource: any[], nombreExcel: string) => {
        ...
        this.dataService.Spinner().show("sp6");  // getExportarExcelAdjudicacionPUN
        this.dataService.Contrataciones().getExportarExcelAdjudicacionEvaluacion(this.request).pipe(
            catchError((e) => of([e])),
            finalize(() => {
    ...
    handleNoAdjudicar = (d) => {
        this.dialogRef = this.materialDialog.open(ModalNoAdjudicarEvalComponent, {
    ...
    handleAdjudicar = (d) => {
        var r = { ...this.request, paginaActual: this.paginatorAdjudicacion.pageIndex, tamanioPagina: this.paginatorAdjudicacion.pageSize };
        this.localStorageService.setItem('_filtrosTmp', JSON.stringify(r));
        this.router.navigate(['./adjudicar-plaza-evaluacion/' + d.idAdjudicacion + '/' + d.idPersona], {
            relativeTo: this.route,
        })
    }
    ...
    handleFinalizarAdjudicacion = () => {
        ...
        this.dataService.Contrataciones().getVerificarcondicionesFinalizarEtapaEvaluacion(data).pipe(
            catchError((e) => of([e])),
            finalize(() => {
                this.dataService.Spinner().hide("sp6");
            }))
            .subscribe((response: any) => {
                if (!response.estaValidado) {
                    this.dataService.Message().msgAutoCloseSuccessNoButton("TODAVÍA CUENTA CON REGISTROS PARA EVALUAR.", 3000, () => {});
                } else {
            ...
            .subscribe((response: any) => {
                if (!response.estaValidado) {
                    ...
                    this.dataService.Message().msgConfirm('¿ESTÁ SEGURO DE QUE FINALIZAR ETAPA?', () => {
                        ...
                        console.log("datos foinalizar etapa:",d);
    ...
    buildShared() {
        this.sharedService.setSharedBreadcrumb("Contratación / Adjudicaciones de Resultados por Evaluacion");
    ...
    load(data: any, pageIndex, pageSize, firstTime=false): void {
        ...
        if (data.idEtapaProceso === null) {
            ...
            .subscribe((d: any) => {
                ...
                if (((d || []).length === 0 || this.totalregistro === 0) && !firstTime) {
