...
export class RegistrarRechazoComponent implements OnInit {
    ...
    handleRechazarPlazas = () => {
        ...
        if (detalle === '' || detalle === null) {
            this.dataService.Message().msgWarning('"INGRESE UNA DESCRIPCIÓN DEL MOTIVO DE RECHAZO DE LAS PLAZAS"', () => {});
            return;
        }
        ...
        this.dataService.Message().msgConfirm('¿ESTÁ SEGURO QUE DESEA RECHAZAR EL LISTADO DE LAS PLAZAS?', 
            () => {
                ...
                this.dataService.Contrataciones().rechazarConsolidadoPlazas(this.request).pipe(
                    catchError((e) => of(e)),
                    finalize(() => {
