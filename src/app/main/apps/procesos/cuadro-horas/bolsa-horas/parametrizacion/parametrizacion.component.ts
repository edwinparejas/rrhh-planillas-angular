...
@Component({
  selector: 'minedu-parametrizacion',
  templateUrl: './parametrizacion.component.html',
  styleUrls: ['./parametrizacion.component.scss'],
  encapsulation: ViewEncapsulation.None,
  animations: mineduAnimations,
})
export class ParametrizacionComponent implements OnInit, OnDestroy, AfterViewInit {
  form: FormGroup;
  loading: false;
  export = false;
  now = new Date();
...
displayedColumns: string[] = [
  ...
  'descripcionUbigeo',
  'totalHoraAsignadaIIEE',
  'totalSadoHoraAsignadaIIEE', 
  'descripcionEstado',
  'acciones'
];
...
etapa: ProcesoEtapaCabResponseModel = new ProcesoEtapaCabResponseModel();
  ...
  ngOnInit(): void {
    ...
    this.idUgel = this.route.snapshot.params.idUgel;
    console.log( this.idUgel );
    this.buildForm();
...
add = (vaction) => { 
    ...
    this.dialogRef = this.materialDialog.open(AgregarInstitucionComponent, {
                   
                   panelClass: 'minedu-agregar-institucion-dialog',
                   width: '1000px',
                   disableClose: true,
                   data: {
                       action: vaction,
...
edit = (vaction,row) => { 
    ...
    this.dialogRef = this.materialDialog.open(AgregarInstitucionComponent, {
                   ...
                   data: {
                       action: vaction,
                       dataKey:  {
                                    idUgel:row.idUgel,//idSubUnidadEjecutora,//this.idUgel,
                                    idInstitucionEducativa:row.idInstitucionEducativa,
                                    idBolsaHora:row.id,
                                    anio:row.anio,
                                    instanciaText:row.descripcionDre,
                                    subInstanciaText:row.descripcionUgel
                                 },
                       parent:this
                   },
               }
...
buscarBolsasHoras = (fistTime: boolean = false,showMessage:boolean=true) => {
 this.setRequest();
if(this.request.anio==null || this.request.anio==0){
    this.dataService.Message().msgAutoCloseWarningNoButton('Seleccione año',this.tiempoMensaje,() => { });
    return;
...
.pipe(
  ...
  console.log(response);
    ...
    this.buscarBolsasHoras(false,false); 
...
setRequest = () => {
  ...
  loadSubInstancia = (dataSubInstancia) => {
    ...
    getTotalHorasResumen = () => {
        ...
        this.dataService.CuadroHoras()
            .getTotalesResumen({
                                anio:this.request.anio,
                                idSubUnidadEjecutora:this.request.idSubInstancia
                               })
            .pipe(
                catchError((e) => { 
                    this.dataService.SnackBar().msgError(CUADRO_HORA_MESSAGE.EXCEPTION_ERROR,
  ...
  handleAprobar = () => {
    ...
    if(this.request.anio==null){
        this.dataService.Message().msgAutoCloseWarningNoButton('Seleccione año para aprobar las bolsas de hora',this.tiempoMensaje,() => { });
        return;
    }
    if(this.request.idSubInstancia===0){
        this.dataService.Message().msgAutoCloseWarningNoButton('Seleccione Sub Instancia para aprobar las bolsas de hora.',this.tiempoMensaje,() => { });
        return;
    }
    ...
    Swal.fire({
        ...
        if (result.value) {
            ...
            let data={
                idUgel: this.request.idSubInstancia,
                anio:this.request.anio
                //idEstado: this.estadoCuadroHoras.APROBADO
                }
    
    this.dataService.Spinner().show('sp6');
    this.dataService
        .CuadroHoras()
        .cambiarEstadoBolsaHoras(data)
        .pipe(
            catchError((e) => { 
                this.dataService.SnackBar().msgError(CUADRO_HORA_MESSAGE.EXCEPTION_ERROR,
                                                    SNACKBAR_BUTTON.CLOSE); return of(e); }),
  ...
  handleExportar = () => {
    ...
    this.dataService
        ...
        .subscribe((response: any) => {
            if (response) {
                descargarExcel(response, "BolsaHoras.xlsx");
