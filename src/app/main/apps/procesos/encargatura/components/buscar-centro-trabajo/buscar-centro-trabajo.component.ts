...
import { TablaTipoCentroTrabajo, TablaRolPassport, PassportTipoSede } from '../../_utils/constants'; 
...
ngOnInit(): void {
  ...
  obtenerInstanciaSubinstanciaDefault() {
    ...
    if (this.data.codigoSede != null) {
      this.dataService.Encargatura().getDefaultValueComboInstanciasByCodSede(this.data.codigoSede).pipe(
        catchError(() => of([])),
            finalize(() => { })
      )
...
ngOnDestroy(): void { }
  ...
  loadNivelesEducativos = (idModalidadEducativa: number) => {
    ...
    this.dataService
      .Encargatura()
      .getNivelEducativo(request.idModalidadEducativa)
        .pipe(
        catchError(() => of([])),
            map((response: any) => response)
        )
