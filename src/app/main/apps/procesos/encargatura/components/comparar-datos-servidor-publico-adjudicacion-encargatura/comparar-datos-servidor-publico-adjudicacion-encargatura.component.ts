...
@Component({
    selector: "minedu-comparar-datos-servidor-publico-adjudicacion-encargatura",
    templateUrl: "./comparar-datos-servidor-publico-adjudicacion-encargatura.component.html",
    styleUrls: ["./comparar-datos-servidor-publico-adjudicacion-encargatura.component.scss"],
    encapsulation: ViewEncapsulation.None,
    animations: mineduAnimations,
})
export class CompararDatosServidorPublicoAdjudicacionEncargaturaComponent implements OnInit {    
    @Input() idAdjudicacion : number;
    ...
    handleSave(){
        ...
        this.dataService.Encargatura().ActualizarDatosServidorPublico(request).pipe(catchError((error) => {
            ...
            if(result!=null){
                if(result.actualizo){
                    this.handleEvent.emit({event:'actualizo', mensaje: "LOS DATOS DEL SERVIDOR PUBLICO"});
                }
                else{
...
export class DatosPlazaDataSource extends DataSource<any> {
    ...
    load(data: any, pageIndex, pageSize, firstTime = false): void {
        ...
        console.log(data);
        this.dataService.Encargatura().DatosServidorPublico(data).pipe(catchError((error) => {
