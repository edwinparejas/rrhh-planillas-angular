...
import { DocumentoSustentoModel } from '../../models/licencia.model';
...
export class DocumentosSustentoComponent implements OnInit {
    ...
    buildForm(): void {
        this.form = this.formBuilder.group({
            ...
            sumilla: [null],
            // codigoDocumentoSustento: [null, [Validators.required, Validators.maxLength(100)]],
            codigoDocumentoSustento: [null, [Validators.maxLength(100)]],
            vistoProyecto: [null, Validators.required],
        });
    }
    ...
    asignarData = (row) => {
        ...
        model.vistoProyecto = row.vistoProyecto == '1' ? true : false;
        model.descripcionVistoProyecto = row.vistoProyecto == '1' ? 'SI' : 'NO';
    ...
    handleAggregate = (form) => {
        ...
        console.log(model)
        const mensajeConfirmacion = '¿Está seguro que desea agregar la información a la grilla?.';
