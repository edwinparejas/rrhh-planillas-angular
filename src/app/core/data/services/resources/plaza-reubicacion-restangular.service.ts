...
import { RestangularBasePath } from 'app/core/data/base-path/restangular-base-path';
...
export class PlazaReubicacionRestangularService {
  ...
  constructor(
    ...
    this.baseUrl = basePath.plazaReubicacionApi;
  }
  ...
  one(path: string, id: string): PlazaReubicacionRestangularService {
    const restangular = this.clone();
    restangular.baseUrl += (path ? '/' + path : '') + '/' + id;
    return restangular;
  }

  all(path: string): PlazaReubicacionRestangularService {
    const restangular = this.clone();
  ...
  download(queryParams?: HttpParams): Observable<Response> {
    return this._http.get(this.baseUrl, { params: queryParams, responseType: 'blob' }).pipe(
      catchError(this.handleError),
      map((response) => {
        return response as any;
      })
  ...
  clone(): PlazaReubicacionRestangularService {
    return new PlazaReubicacionRestangularService(this._http, this.router, { plazaReubicacionApi: this.baseUrl } as RestangularBasePath);
  }
