...
export class PlazaAdecuacionRestangularService { 
  ...
  constructor(
    ...
    this.baseUrl = basePath.plazaAdecuacionApi;
  }
  ...
  one(path: string, id: string): PlazaAdecuacionRestangularService {
    const restangular = this.clone();
    restangular.baseUrl += (path ? '/' + path : '') + '/' + id;
    return restangular;
  }

  all(path: string): PlazaAdecuacionRestangularService {
    const restangular = this.clone();
  ...
  clone(): PlazaAdecuacionRestangularService {
    return new PlazaAdecuacionRestangularService(this._http, this.router, { plazaAdecuacionApi: this.baseUrl } as RestangularBasePath);
  }
