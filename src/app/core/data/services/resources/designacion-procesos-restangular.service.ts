...
export class DesignacionProcesosRestangularService {
  ...
  constructor(
    ...
    this.baseUrl = basePath.gestionProcesosApi;
  }
  ...
  one(path: string, id: string): DesignacionProcesosRestangularService {
    const restangular = this.clone();
    restangular.baseUrl += (path ? "/" + path : "") + "/" + id;
    return restangular;
  }

  all(path: string): DesignacionProcesosRestangularService {
    const restangular = this.clone();
  ...
  clone(): DesignacionProcesosRestangularService {
    return new DesignacionProcesosRestangularService(this._http, this.router, { gestionProcesosApi: this.baseUrl } as RestangularBasePath);
  }
