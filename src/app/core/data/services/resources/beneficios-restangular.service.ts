...
export class BeneficiosRestangularService {
    ...
    {
        this.baseUrl = basePath.beneficiosApi;
    }
    ...
    one(path: string, id: string): BeneficiosRestangularService {
        const restangular = this.clone();
        restangular.baseUrl += (path ? '/' + path : '') + '/' + id;
        return restangular;
    }

    all(path: string): BeneficiosRestangularService {
        const restangular = this.clone();
    ...
    delete(queryParams: HttpParams): Observable<Response> {
        
        return this._http.delete(this.baseUrl,{ params: queryParams }).pipe(
            catchError(this.handleError),
            map((response) => {
                return response as any;
            })
    ...
    clone(): BeneficiosRestangularService {
        return new BeneficiosRestangularService(this._http, this.router, {
            beneficiosApi: this.baseUrl,
